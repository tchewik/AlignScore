...
class BERTAlignModel(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        seq_relationship_score = self.bin_layer(
            self.dropout(base_model_output.pooler_output))  ## pooled output for classification
        ...
        if 'mlm_label' in batch.keys():  ### 'mlm_label' and 'align_label' when training
            ...
            masked_lm_loss = ce_loss_fct(prediction_scores.view(-1, self.base_model.config.vocab_size),
                                         batch['mlm_label'].view(-1))  # / self.con vocabulary
            next_sentence_loss = ce_loss_fct(seq_relationship_score.view(-1, 2),
                                             batch['align_label'].view(-1)) / math.log(2)
        ...
        return ModelOutput(
            loss=total_loss,
            all_loss=[masked_lm_loss, next_sentence_loss, tri_label_loss,
                      reg_label_loss] if 'mlm_label' in batch.keys() else None,
            loss_nums=[masked_lm_loss_num, next_sentence_loss_num, tri_label_loss_num,
                       reg_label_loss_num] if 'mlm_label' in batch.keys() else None,
    ...
    def electra_forward(self, batch):
        if 'mlm_label' in batch.keys():
            ...
            masked_lm_loss = ce_loss_fct(generator_output.view(-1, self.generator.config.vocab_size),
            ...
            hallucinated_tokens[batch['mlm_label'] != -100] = torch.argmax(generator_output, dim=-1)[
                batch['mlm_label'] != -100]
            replaced_token_label = (batch[
                                        'input_ids'] == hallucinated_tokens).long()  # .type(torch.LongTensor) #[batch['mlm_label'] == -100] = -100
        ...
        if 'mlm_label' in batch.keys():  ### 'mlm_label' and 'align_label' when training
            ...
            total_loss.append(
                0.2 * masked_lm_loss if (not torch.isnan(masked_lm_loss).item() and self.need_mlm) else 0.)
        ...
        return ModelOutput(
            ...
            all_loss=[masked_lm_loss, next_sentence_loss, tri_label_loss, reg_label_loss,
                      hallu_detect_loss] if 'mlm_label' in batch.keys() else None,
